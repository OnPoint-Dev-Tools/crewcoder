...
- Coordinated releases use `npm run release -- <major.minor.patch>` to bump the umbrella, agent, browser client, and SDK together while preserving the independent TUI version. Keep `scripts/release.cjs` synchronized with those manifests, exact internal dependency pins, runtime version constants, generated version API baselines, the root lockfile, and the protected npm publishing workflow. Automatic API baseline refreshes may only change the client and SDK `api/version.d.ts` files.
