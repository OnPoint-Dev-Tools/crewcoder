...
`Renderer.start()` sets the terminal background color with OSC 11 so terminal padding matches the active theme, and `Renderer.stop()` resets it with OSC 111. Conversation view commits settled transcript lines into the terminal scrollback and only rewrites the live tail (streaming block, working indicator, composer). Mouse reporting is off in that mode so Ghostty/Kitty/xterm own wheel and selection; it turns back on for home, modal overlays, the sidebar, and Live UI. `CREWCODER_TUI_MOUSE=0` still forces mouse reporting off everywhere. See `docs/TRANSCRIPT.md`.
...
- Conversation view uses the terminal's scrollback as the transcript history. `MainViewport.layoutTranscript` caches per-block layout and splits settled lines from the live tail; `Renderer` appends only the settled delta and paints only newly committed rows. Do not go back to painting the full history every tick. The mutable live tail is bounded to the available viewport height so expanded running-tool output cannot grow the rewrite region without limit. Full-screen surfaces (sidebar and overlays) show the latest transcript snapshot; closing them resumes the same committed terminal history. There is no CrewCoder-owned transcript pager, scrollbar pill, or diff-hunk jump navigation. Native conversation scrolling must stay usable during an active run by not capturing wheel/PageUp.
...
- The viewport `image` block renders real terminal pixels on Kitty/Ghostty/iTerm2 and a metadata chip everywhere else. Protocol detection + encoders live in `src/tui/image-protocol.ts` (`CREWCODER_TUI_IMAGE_PROTOCOL=kitty|iterm|none|auto`); `src/tui/renderer.ts` draws them in a post-frame graphics pass so the line-diff renderer stays text-first. Only fully visible blocks are drawn. Conversation scrollback draws graphics for images still in the live tail; settled image blocks keep their text caption in history. A diff render must only erase and redraw graphics when the image signature changed or a repainted row overlaps the image rectangle; erasing on every frame made settled images flicker against the 90ms spinner / 120ms render tick. Guarded by `src/tests/renderer.test.ts`.
- Full-screen frames (home, sidebar, modals) treat `MainViewport` placement rows as absolute terminal rows. Scrollback frames place live-tail images relative to the live region start. Opaque modal compositing must remove any placement intersecting the modal rectangle because terminal graphics render above text and cannot be clipped; the placement returns after the modal closes. Guarded by `src/tests/image-wiring.test.ts`.
