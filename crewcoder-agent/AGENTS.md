...
- Provider-native compaction is observable lifecycle, not merely a token-count change. Codex app-server `contextCompaction` item start/completion and legacy `thread/compacted` notifications must emit one deduplicated provider-compaction lifecycle; Claude `compact_boundary` must emit provider completion. ACP projects these through `_crewcoder/compaction_update` without claiming the CrewCoder durable transcript itself was rewritten.
- Nested Codex compaction must use CrewCoder's resolved policy instead of app-server's smaller model default. Pass the known `contextWindow`, computed model-relative `autoCompactTokenLimit`, and `total` scope as process-only Codex app-server configuration; never persist these overrides into the user's Codex config. Sol, Terra, and Luna GPT-5.6 variants are all 1,050,000-token models and therefore compact at 630,000 tokens when normal auto-compaction is enabled.
- Every shipped built-in model must declare a positive `modelCatalog[].contextWindow`; keep the model-id and catalogue lists exact and regression-tested. Provider-reported runtime windows are authoritative and must update both usage state and the active percentage threshold. Keep Claude Agent SDK native auto-compaction disabled so it cannot compact its opaque resumed session before CrewCoder's durable provider-neutral policy.
...
- Grok ACP `usage_update.used` is live occupancy and `usage_update.size` is the authoritative runtime context window. Preserve both; never substitute cumulative cost or an estimated token count. Runtime window changes must recalculate the CrewCoder threshold.
